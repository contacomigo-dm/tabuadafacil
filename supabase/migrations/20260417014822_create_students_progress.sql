-- Students table: one row per student (identified by first name)
...
-- Per-question attempts (used to compute per-table error stats)
...
-- Sessions (each time a student plays)
...
-- Teacher settings (single-row config for password)
...
-- Enable RLS, permissive policies (no auth in this app — access controlled in UI by teacher password)
